class SkinSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2, 18, `ELEGÍ EL SKIN DE ${this.champion.name.toUpperCase()}`, {
      fontSize: '17px', fontFamily: 'Arial Black', color: '#ffd200', stroke: '#7a3b00', strokeThickness: 4,
    }).setOrigin(0.5);
    ...
    SKINS.forEach((skin, i) => {
      ...
      const key = generateSkinPreview(this, playerWithSkin, 96);
      ...
      const paint = (fillColor, fillA, strokeColor) => {
        ...
        card.lineStyle(2, strokeColor, 1);
        card.strokeRoundedRect(x - cardW / 2, y - cardH / 2, cardW, cardH, 10);
      };
      paint(0x1c2a5e, 0.85, 0x33427a);

      this.add.image(x, y - 12, key).setScale(0.34);
      this.add.text(x, y + 32, skin.name, {
        fontSize: '9px', fontFamily: 'Arial', color: '#ffffff', align: 'center', wordWrap: { width: cardW - 6 },
      }).setOrigin(0.5);
      this.add.text(x + cardW / 2 - 10, y - cardH / 2 + 10, skin.emoji, { fontSize: '13px' }).setOrigin(0.5);
    ...
    this.add.text(width / 2, height - 14, 'Tocá un skin para continuar', {
